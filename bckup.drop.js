(function () {
    //document.body.appendChild(ifrm); //laittaa iframen alas bodyy
    ...
    function createIframe() {
        ...
        //document.querySelector("#twilight-sticky-header-root > div")
        var dropsBody = ifrm.contentWindow.document.getElementsByTagName("BODY")[0];
        dropsBody.style.visibility = "hidden";
        

        //var dropsDiv = ifrm.contentWindow.document.getElementsByClassName("inventory-max-width")[0]; //Droppi divi
        //dropsDiv.style.visibility = "visible";

        var header = ifrm.contentWindow.document.querySelector("div.title-bar.tw-flex.tw-pd-b-1")
        ...
        var availableDrops = ifrm.contentWindow.document.getElementsByClassName("tw-flex tw-flex-column tw-mg-t-2")

      //  for (j = 0; j < availableDropsContainers.length; j++){

            for (i = 0; i < availableDrops.length; i++){
                if(availableDrops[i].getElementsByClassName("tw-visible").length > 0){
                    console.log("Drop nr." + i)
                    availableDrops[i].style.margin = "0"
                    availableDrops[i].style.marginRight = "15px"
                    availableDrops[i].style.width = "160px"
                    header.appendChild(availableDrops[i])
                }
                else{
                    if(availableDrops[i].getElementsByClassName("tw-align-items-center tw-core-button-icon tw-inline-flex").length > 0){
                        ...
                        availableDrops[i].style.marginRight = "15px"
                        availableDrops[i].style.width = "160px"
                        header.prepend(availableDrops[i])
                    }
                }
            }
   // }
    ...
    ifrm.setAttribute('src', 'https://www.twitch.tv/drops/inventory');
    ifrm.width = "100%"
    ifrm.height = "500px" //280px
    ifrm.style.visibility = "hidden" //Piilota iframe

    insertIframe = setInterval(function () {
