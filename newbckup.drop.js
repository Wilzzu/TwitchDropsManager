(function () {
    ...
    function createIframe() {
        ...
        //Scrollable var header = ifrm.contentWindow.document.querySelector("div.tw-flex-grow-1.tw-mg-b-2")
        ...
        for (j = 0; j < availableDropsContainers.length; j++){
            ...
            for (i = 0; i < availableDrops.length; i++){    //Chekkaa monta containerii on ja käy jokase läpi
                ...
                if(availableDrops[i].getElementsByClassName("tw-visible").length > 0){ //Monta droppia on containeri sisäl
                    ...
                    link.style.color = "#efeff1"
                    link.style.paddingBottom = "5px"

                    let game = link.href.split('game/').pop().split('?')[0]     //Otetaa pelin linkki jos ei oo striimeri
                    if(link.innerHTML == "a participating live channel"){
                        if(game.length > 0){ 
                            link.innerHTML = game                               //Muutetaa teksti pelin nimeks
                        }
                        else{
                    ...
                    header.appendChild(availableDrops[i])                      //Lisätää koko juttu headerii
                }
                    ...
                    if(availableDrops[i].getElementsByClassName("tw-align-items-center tw-core-button-icon tw-inline-flex").length > 0){
                        ...
                        let game = link.href.split('game/').pop().split('?')[0]
                    if(link.innerHTML == "a participating live channel"){
                        if(game.length > 0){
                            link.innerHTML = game
                        }
